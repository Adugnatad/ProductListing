...
const TableListing = () => {
  ...
  useEffect(() => {
    ...
    const data = dataToSearch.filter((row) =>
    ...
    <>
      ...
      <TableContainer mx={{ md: 10 }}>
          ...
          <Tbody id="tableBody">
            {currentRows.map((data) => (
              <Tr key={data.id} id={data.id}>
                <Td>
                  <Checkbox onChange={() => selectRow(data.id)} />
                </Td>
                <Td cursor="pointer" onClick={() => selectRow(data.id)}>
                  {data.name}
                </Td>
                <Td>{data.brand}</Td>
                <Td>{data.id}</Td>
        ...
        <Box
          ...
          w="100%"
        >
          <Text fontSize={15} color="" my={3}>
            Showing {indexOfFirstRow} to {indexOfLastRow} of {tableData.length}{" "}
            results
          </Text>
          <Flex direction="row" align="center">
            <Button variant="outline" mr={3} onClick={() => paginate("back")}>
              Previous
            </Button>
            <Button variant="outline" onClick={() => paginate("forward")}>
              Next
            </Button>
          </Flex>
        </Box>
